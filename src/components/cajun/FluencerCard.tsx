...
export function FluencerCard({ fluencer }: { fluencer: CajunFluencer }) {
  ...
  const [imgSrc, setImgSrc] = useState(fluencer.profilePhoto || "/placeholder.svg");
  ...
  return (
    <article className="card-lift overflow-hidden rounded-[12px] border border-[var(--spanish-moss)]/30 bg-white shadow-sm">
      <div className="relative aspect-[4/3] bg-[var(--cream)]">
        <Image src={imgSrc} alt={fluencer.name} fill className="object-contain p-2" onError={() => setImgSrc("/placeholder.svg")} />
